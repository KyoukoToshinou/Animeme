...
#define FL_PRONE 0x8
		...
		typedef struct
		{
			...
			Vector			refdefViewAngles; // vec3 only here
		}refdef_t;
		...
		typedef struct // vec3 class too
		{
			__int32 currentValid; //0x0000 
			char _0x0004[24];
			Vector lerpOrigin; //0x001C 
			Vector lerpAngles; //0x0028 
			char _0x0034[64];
			Vector oldOrigin; //0x0074 
			char _0x0080[76];
			__int32 clientNum; //0x00CC 
			__int32 eType; //0x00D0 
			DWORD eFlags; //0x00D4 
			char _0x00D8[12];
			Vector newOrigin; //0x00E4 
			char _0x00F0[160];
		...
		typedef struct
		{
			...
			Vector	start;			// 28
			Vector	end;			// 40
			Vector	viewDirection;	// 52 // vec3 here too
		}bulletTrace_t;
		...
		*/
		...
		typedef struct {
			...
			float aX; //0x003C  
			float aY; //0x0040  
			char unknown68[348]; //0x0044
			__int32 health; //0x01A0  
		} Actor_t;

		void RotatedPic(float, float, float, float, float, float*, int);
		void StretchPic(float, float, float, float, float, float, float, float, float*, int);
		void Shader(float, float, float, float, float*, int);
		void String(float, float, void*, char*, float*, float, float, float, float);

		extern int iWhiteShader;
		typedef int(*RegisterShader_t)(char* szShadername, signed int iUnknown);
		extern RegisterShader_t RegisterShader;

		typedef void*(*RegisterFont_t)(char* szFontname, signed int iUnknown);
		extern RegisterFont_t RegisterFont;

		typedef signed int(*EndFrame_t)();
		extern EndFrame_t EndFrame;
	...
	namespace pointers
	{
		extern sdk::clientInfo_t* pClientInfo;
		extern sdk::entity_t* pEntities;
		extern sdk::refdef_t* pRefdef;
		extern void* pFont;
	}

	signed int newEndFrame();
	bool Init();
}